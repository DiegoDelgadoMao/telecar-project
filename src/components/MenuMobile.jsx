            ...
            <ul className={`${(!navMobile)? 'hidden' : ''} z-[2] bg-fourth absolute bottom-[-208px] w-full h-52 left-0 bg-cyan-200 flex flex-col`} >
                {
                    items.map(item => (
                        <li key={item.txt} className='hover:bg-cyan-300'>
                            <a href="#" className='flex flex-row-reverse justify-end py-4 pl-6 gap-x-4 items-center text-fifthLight hover:text-second drop-shadow-FifthLight hover:drop-shadow-second'>
                                <span>{item.icon}</span>
